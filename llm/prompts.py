...
ВНИМАНИЕ — типичные ошибки классификации (НЕ ДОПУСКАЙ их):
- «Настроил очереди сообщений (RabbitMQ/Kafka)» — это автоматизация, НЕ fastChanges
- «Сократил задержку ответа / латенси» — это производительность системы, НЕ fastChanges
- «Контейнеризировал сервис (Docker/K8s)» — это инфраструктура деплоя, НЕ fastChanges
- «Real-time обновления для пользователей (WebSocket/SignalR)» — это UX, НЕ fastChanges
- fastChanges = ТОЛЬКО частота изменений КОДА и циклов РЕЛИЗА (например: «сократил цикл релиза с 2 недель до 2 дней», «выпускали фичи каждый день»)
